...
\title{Sortition Upstream of NTQR}
...
\centering
\vspace*{0.55cm}
{\Huge\sffamily\bfseries Sortition Upstream of NTQR\par}
\vspace{0.4em}
{\Large\sffamily How Panel Formation and Size Shape Ground-Truth-Free Evaluation\par}
\vspace{0.75em}
{\large\sffamily\bfseries Daniel Ari Friedman\par}
{\small\sffamily Active Inference Institute\par}
{\small\sffamily\texttt{danielarifriedman@gmail.com}\par}
{\small\sffamily\href{https://orcid.org/0000-0001-6232-9096}{ORCID: 0000-0001-6232-9096}\par}
\vspace{0.08em}
{\small\sffamily DOI: \href{https://doi.org/10.5281/zenodo.21083779}{10.5281/zenodo.21083779}\par}
\vspace{0.35em}
\makeatletter
{\@date\par}
\makeatother
\vfill
\includegraphics[width=0.98\textwidth,height=0.60\textheight,keepaspectratio]{\detokenize{../figures/ntqr_cover.png}}
\vfill
\end{titlepage}
\thispagestyle{empty}
\tableofcontents
...
axis the lottery does not balance erases the protection (0.147\texorpdfstring{\ensuremath{\to}}{->}0.229).
...
\subsection{Blind evaluation begins before the
estimator}\label{blind-evaluation-begins-before-the-estimator}
...
logic- and constraint-based \citep{platanios2014} and spectral
\citep{parisi2014} --- and, like the exact NTQR solver, these methods
...
for correlated binary outcomes \citep{emrich1991, nelsen2006}, which
...
\subsection{Application review as the empirical stress
test}\label{application-review-as-the-empirical-stress-test}
...
\subsection{Sortition from civic lotteries to evaluator
sampling}\label{sortition-from-civic-lotteries-to-evaluator-sampling}
...
\subsection{Falsifiable claims and negative
controls}\label{falsifiable-claims-and-negative-controls}
...
tests H5. Methods (Table tbl.~\ref{tbl:falsification}) states the
load-bearing assumption and the negative-control check behind each, and
the Discussion returns an explicit verdict hypothesis by hypothesis.
...
\item
  ...
  separation gate and a power budget (fig.~\ref{fig:ranking}).
\item
  ...
  against analytical sign predictions (fig.~\ref{fig:repideo}), and
  resolved by the composition-coupled error confound that fans the
  strategies apart as within-bloc coupling rises
  (fig.~\ref{fig:blocphase}).
\item
  ...
  (fig.~\ref{fig:powercurve}, fig.~\ref{fig:powervn},
  fig.~\ref{fig:triocond}).
\item
  ...
  with a bootstrap interval (fig.~\ref{fig:tolerance}); the recovery
  slope, unresolved by the global-injection sweep, is resolved positive
  once the correlation is composition-coupled and marginal-preserving
  (fig.~\ref{fig:blocphase}).
\item
  ...
  (fig.~\ref{fig:trackinversion}, fig.~\ref{fig:postdocalign}).
\end{enumerate}
...
\subsection{Synthetic deterministic track: seeded panels, blind
estimates, oracle
scoring}\label{synthetic-deterministic-track-seeded-panels-blind-estimates-oracle-scoring}
...
\subsubsection{Pipeline: panel formation precedes no-answer-key
estimation}\label{pipeline-panel-formation-precedes-no-answer-key-estimation}
...
fig.~\ref{fig:pipeline} shows this upstream-to-downstream flow at a
glance.

\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.50\textheight,keepaspectratio,alt={Left-to-right pipeline of the deterministic synthetic instrument (steps 1--5 in text; count tokens annotated from 96, 300, 8 so the figure cannot drift from the reported configuration): a known population is sampled \texorpdfstring{\ensuremath{\to}}{->} a panel is formed by one of four strategies \texorpdfstring{\ensuremath{\to}}{->} the panel judges a key-hidden corpus \texorpdfstring{\ensuremath{\to}}{->} the EIE evaluator runs blind over trios \texorpdfstring{\ensuremath{\to}}{->} only scoring reads the labels. The bracket marks the unsupervised region; the manipulated variable is the upstream formation rule. Explanatory schematic only; quantitative results are in the Results section.}]{../figures/method_pipeline_schematic.png}
...
is sampled \texorpdfstring{\ensuremath{\to}}{->} a panel is formed by one of four strategies \texorpdfstring{\ensuremath{\to}}{->} the panel
judges a key-hidden corpus \texorpdfstring{\ensuremath{\to}}{->} the EIE evaluator runs blind over trios \texorpdfstring{\ensuremath{\to}}{->}
...
\texttt{accuracy\_a\ =\ P(vote\ a\ |{}\ true\ a)} and
\texttt{accuracy\_b\ =\ P(vote\ b\ |{}\ true\ b)}, derived from a
...
with a sign \textbf{correlated with each expert's ideology} (left \texorpdfstring{\ensuremath{\to}}{->}
negative, right \texorpdfstring{\ensuremath{\to}}{->} positive). Because this only shifts each judge's
...
\subsubsection{Panel-formation strategies: four upstream
rules}\label{panel-formation-strategies-four-upstream-rules}
...
\subsubsection{NTQR evaluation: trio EIE, oracle scoring, and majority
voting}\label{ntqr-evaluation-trio-eie-oracle-scoring-and-majority-voting}
...
\subsubsection{Companion diagnostics: alarm cost, ternary feasibility,
and maximin
fairness}\label{companion-diagnostics-alarm-cost-ternary-feasibility-and-maximin-fairness}
...
\subsubsection{Notation: cells, trios, and inferential
units}\label{notation-cells-trios-and-inferential-units}
...
claims from borrowing strength from one another. Table
tbl.~\ref{tbl:notation} is the compact ledger for symbols, estimators,
inferential units, and artifact ownership.
...
\subsubsection{Assumption ledger: how each claim can
fail}\label{assumption-ledger-how-each-claim-can-fail}

The analysis is organized as falsifiable claims rather than a single
success story. Table tbl.~\ref{tbl:falsification} states what would
count against each claim and which artifact carries the check. The rows
map onto the Introduction's hypotheses: the
representative-vs-ideological, panel-size, tolerance-sweep, and
real-Ollama rows are the negative-control checks for H2, H3, H4, and H5
respectively; the NTQR-EIE-recovery row guards the estimator the whole
study depends on (and so underpins H1, the strategy ranking tested
directly in Results); and the null-and-significance row fixes the
design-limited-vs-resolved interpretation discipline applied throughout.
...
(fig.~\ref{fig:blocphase}), not a universal sortition win. \\
...
\subsubsection{Sweep profiles: profiles, seeds, and aggregation
units}\label{sweep-profiles-profiles-seeds-and-aggregation-units}
...
\subsubsection{Controlled-correlation sweep: injected dependence as a
diagnostic}\label{controlled-correlation-sweep-injected-dependence-as-a-diagnostic}
...
\breaktt{independence\_sweep.py} sweeps \(\rho\) \texorpdfstring{\ensuremath{\times}}{x} strategy at the trio
...
\subsubsection{Composition-coupled confound: when group membership
carries shared
error}\label{composition-coupled-confound-when-group-membership-carries-shared-error}
...
sweeps strategy \texorpdfstring{\ensuremath{\times}}{x} \(\rho\) \texorpdfstring{\ensuremath{\times}}{x} bias-spread \texorpdfstring{\ensuremath{\times}}{x} stringency \texorpdfstring{\ensuremath{\times}}{x} panel-size \texorpdfstring{\ensuremath{\times}}{x}
...
\subsubsection{Herfindahl exposure: concentration predicts shared-error
risk}\label{herfindahl-exposure-concentration-predicts-shared-error-risk}
...
(fig.~\ref{fig:dial}). The contribution is thus a closed-form chain ---
...
\subsubsection{Statistical power: separating rankings from resolved
contrasts}\label{statistical-power-separating-rankings-from-resolved-contrasts}
...
\subsection{Real-Ollama reviewer-panel track: single-model live
companion}\label{real-ollama-reviewer-panel-track-single-model-live-companion}
...
\subsubsection{Postdoctoral corpus: protected-attribute stress
test}\label{postdoctoral-corpus-protected-attribute-stress-test}
...
\subsubsection{Reviewer profiles: expertise and age-bias
prompts}\label{reviewer-profiles-expertise-and-age-bias-prompts}
...
\subsubsection{Postdoc aggregation: analytical-vs-Gemma
alignment}\label{postdoc-aggregation-analytical-vs-gemma-alignment}
...
\subsection{Synthetic deterministic results: controlled spine for
H1-H4}\label{synthetic-deterministic-results-controlled-spine-for-h1-h4}
...
profile only. Throughout, a \emph{regime} is one (expert-stringency \texorpdfstring{\ensuremath{\times}}{x}
bias-spread \texorpdfstring{\ensuremath{\times}}{x} panel-size) cell of the sweep grid (sixteen cells in the
active profile); contrasts are evaluated cell by cell and averaged only
where the text says so.

\subsubsection{Formation strategy sets the recovery
floor}\label{formation-strategy-sets-the-recovery-floor}
...
far apart and the other three collapse together
(fig.~\ref{fig:ranking}). Competence-first selection recovers at mean
EIE error 0.037 --- roughly a quarter of the error of any other rule ---
while representative sortition, random selection, and single-bloc
ideological selection are \textbf{statistically indistinguishable from
one another}, clustered at 0.147--0.148:
...
confidence intervals in the table above are disjoint (0.037 \texorpdfstring{\ensuremath{\pm}}{+/-} 0.003
versus 0.147 \texorpdfstring{\ensuremath{\pm}}{+/-} 0.014), and the power analysis resolves it as
...
\subsubsection{Sortition only separates when the confound rides on the
balanced
axis}\label{sortition-only-separates-when-the-confound-rides-on-the-balanced-axis}

\textbf{H2 (concentrating correlated error degrades recovery).}
fig.~\ref{fig:repideo} reports the single-bloc-minus-representative EIE
...
postdoc companion in fig.~\ref{fig:postdocrank},
fig.~\ref{fig:postdocbias}, and
fig.~\ref{fig:postdocalign}.}\label{fig:repideo}
\end{figure}
...
\subsubsection{NTQR beats majority voting only in selected
regimes}\label{ntqr-beats-majority-voting-only-in-selected-regimes}
...
The companion alignment map (fig.~\ref{fig:theoryalign}) makes the
...
\subsubsection{Larger panels are a neutral sampling knob
here}\label{larger-panels-are-a-neutral-sampling-knob-here}

\textbf{H3 (size is a sampling knob, not a uniform improvement).}
fig.~\ref{fig:powercurve} plots EIE error against panel/ensemble size
for each strategy. If size were a clean power knob, every curve would
fall from size 3 to size 6. It does not:
...
trios. A per-trio diagnostic refutes that guess
(fig.~\ref{fig:triocond},
...
\subsubsection{Global injected correlation is measurable but
recovery-limited}\label{global-injected-correlation-is-measurable-but-recovery-limited}
...
(fig.~\ref{fig:blocphase}). The global-injection sweep remains in the
paper as the correlation \emph{diagnostic} (the realized correlation
does rise with \texttt{rho}) with its accuracy artifact disclosed. This
is a \textbf{measured behaviour under controlled correlation}, validated
in simulation only; it does \emph{not} show that sortition restores low
...
\subsubsection{Composition-coupled correlation exposes the sortition
mechanism}\label{composition-coupled-correlation-exposes-the-sortition-mechanism}
...
stringency, panel-size, and seed regimes (fig.~\ref{fig:blocphase}).
...
CI \texorpdfstring{\ensuremath{\pm}}{+/-}0.012). Nor is the averaged subsample cherry-picked by the
...
error: 1.000; fig.~\ref{fig:dial}). This is the closed-form Herfindahl
...
as the expertise-tier negative control in fig.~\ref{fig:blocphase}
shows.}\label{fig:dial}
\end{figure}

\subsubsection{Power budgets distinguish ranking from resolved
contrasts}\label{power-budgets-distinguish-ranking-from-resolved-contrasts}
...
significant win. fig.~\ref{fig:powervn} shows the analytic
...
\subsubsection{Companion diagnostics bound cost, correlation, fairness,
and
consistency}\label{companion-diagnostics-bound-cost-correlation-fairness-and-consistency}

The companion alarm's answer-key enumeration is roughly cubic in corpus
size (fig.~\ref{fig:alarm}): about 0.7 s at \(Q=20\), 8.9 s at \(Q=50\),
and 97.9 s at \(Q=100\) (measured by \breaktt{scripts/bench\_alarm.py},
...
formation strategy induces (fig.~\ref{fig:strategycorr}): single-bloc
...
population (fig.~\ref{fig:fairness}) --- the maximin objective is the
floor on who can be seated, independent of any downstream recovery
number. The N-judge alarm-power track records a saturated small-\(Q\)
alarm-firing rate across the plotted panel sizes
(fig.~\ref{fig:alarmpower}). The ternary (\(R=3\)) track is
consistency/feasibility only --- it confirms three-way vote profiles
satisfy the NTQR axioms and is never an \(R=3\) recovery claim (out of
scope) --- so it yields a pass/fail check rather than a plotted number.

\begin{figure}
\centering
\includegraphics[width=0.7\linewidth,height=0.50\textheight,keepaspectratio,alt={Bar chart of the mean realized pairwise error correlation that each panel-formation strategy induces among its judges, measured by NTQR's own supervised estimator over the tolerance sweep, from source output/data/independence\_sweep.csv. Higher bars mean the strategy seats judges whose mistakes are more correlated, which is precisely the error-independence assumption the exact trio solver leans on. Single-bloc selection is the tallest bar, consistent with its design as the deliberately correlated comparator, while representative and random draws sit lower. Read this as a structural property of the draw itself, upstream of any recovery number. Caveat: this is a structural diagnostic of the formed panel, not a recovery-effect claim about downstream EIE error.}]{../figures/strategy_correlation.png}
...
\subsection{Real-Ollama postdoctoral panel results: live H5
companion}\label{real-ollama-postdoctoral-panel-results-live-h5-companion}
...
\subsubsection{Gemma ranking asks the same sampling question under
prompt
labels}\label{gemma-ranking-asks-the-same-sampling-question-under-prompt-labels}
...
\subsubsection{Same-bias panels expose age-conditioned
recommendations}\label{same-bias-panels-expose-age-conditioned-recommendations}
...
\subsubsection{Analytical and Gemma cells stay juxtaposed, not
pooled}\label{analytical-and-gemma-cells-stay-juxtaposed-not-pooled}
...
\subsubsection{Synthetic strategy ranking does not transfer to the live
track}\label{synthetic-strategy-ranking-does-not-transfer-to-the-live-track}
...
three-seat panel grain (fig.~\ref{fig:trackinversion}) the strategy
...
\subsection{Hypothesis verdicts before
interpretation}\label{hypothesis-verdicts-before-interpretation}
...
\item
  ...
  channel (fig.~\ref{fig:blocphase}), the contrast resolves cleanly:
  single-bloc error exceeds representative error in 180/205 matched
  regimes (paired mean 0.102, 95\% CI \texorpdfstring{\ensuremath{\pm}}{+/-}0.012), the gap widening from
...
\item
  ...
  (fig.~\ref{fig:blocphase}), with far more observations per point,
  resolves the recovery half in the affirmative: recovery error rises
  with within-bloc coupling for the panels that concentrate the
  confound, and the closed-form Herfindahl account (§Methods) explains
  the ordering. So the recovery half is a result, not an open question,
  under a correctly specified confound.
\item
  \textbf{H5 (the synthetic ranking transfers to a live single-model
...
\subsection{Practical lesson: selection rule before panel
size}\label{practical-lesson-selection-rule-before-panel-size}
...
\subsection{Formation strategy is the measured
lever}\label{formation-strategy-is-the-measured-lever}
...
\subsection{Design-limited nulls remain
results}\label{design-limited-nulls-remain-results}
...
\item
  ...
  H2 verdict and fig.~\ref{fig:blocphase}); the null is a property of
  the baseline design, not of sortition.
\item
...
\subsection{Independence explains why strategy ordering
changes}\label{independence-explains-why-strategy-ordering-changes}
...
error-correlation channel (fig.~\ref{fig:blocphase}). Competence-first
...
\subsection{Error independence must be measured before
interpretation}\label{error-independence-must-be-measured-before-interpretation}
...
sweep (fig.~\ref{fig:blocphase}) resolves the recovery half in the
...
\subsection{Scholarship frames the stress test, not the evidence
level}\label{scholarship-frames-the-stress-test-not-the-evidence-level}
...
\subsection{Limitations: synthetic scope, single-model live evidence,
historical
analogy}\label{limitations-synthetic-scope-single-model-live-evidence-historical-analogy}
...
\subsection{Synthetic and live tracks operate at different inference
levels}\label{synthetic-and-live-tracks-operate-at-different-inference-levels}
...
\subsection{Data, code, and generated-artifact
availability}\label{data-code-and-generated-artifact-availability}
...
\subsection{Ethics, protected attributes, and competing
interests}\label{ethics-protected-attributes-and-competing-interests}
