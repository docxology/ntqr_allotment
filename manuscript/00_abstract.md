...
On a fully deterministic instrument ({{N_SEEDS}} seeds, {{N_EXPERTS}} experts,
{{N_ITEMS}} items), the dominant lever is *which* rule forms the panel, not its size:
competence-first selection recovers best ({{RANK_EXPERTISE_THRESHOLD_EIE}}), while
...
flat while single-bloc selection degrades, the gap widening from {{BLOC_SEP_LO}} to
{{BLOC_SEP_HI}}. Within this instrument the relationship is closed-form: recovery
...
not balance erases the protection ({{BLOC_CTRL_REP_LO}}→{{BLOC_CTRL_REP_HI}}). The
...
({{POSTDOC_MODEL}}) the synthetically-best competence-first rule was the worst,
